...
class LoE(nn.Module):
    ...
    def __init__(
        ...
        self.num_layers = self.loe_configs.num_layers
        self.indices = None
        self.img_shape = self.loe_configs.img_dim
        self.tile_dims = torch.tensor(list(self.loe_configs.tile_dims))
        self.masks = []
        self.tile_indices = []
        self.n_data = 0
    ...
    def forward(self, x, flex=False, labels=None):
        ...
        for i, mod in enumerate(self.net):
            ...
            x = mod(x, self.tile_indices[i], flex=flex)

        out = self.untiling(x, self.indices, self.n_data)
    ...
    def get_tile_indices(self, x, flex):
        ...
        for i in range(len(self.net)):
            ...
            self.tile_indices.append(torch.tensor([idx for idx in range(torch.prod(self.tile_dims[i]))]).to(x.device))
    ...
    def get_indices(self, x, tile_dims, img_shape=None):
        ...
        weight_indices = None
        for tile_dim in range(tile_dims):
